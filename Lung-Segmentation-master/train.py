...
# train and validation add vis
def run(model, train_loader, val_loader, criterion):
    best_iou = 0
    for epoch in range(1, opt.epochs+1):
        train(model, train_loader, criterion, epoch)
        avg_iou = val(model, val_loader, criterion, epoch)
        if avg_iou > best_iou:
            best_iou = avg_iou
            if opt.save_model:
                save_path = '/content/drive/My Drive/Lung_segmentation_results/Checkpoints/RSNA_UNET'
...
if __name__ == '__main__':
    ...
    if opt.is_train:
        ...
        model.load_state_dict(torch.load(os.path.join(opt.checkpoint_dir, 'RSNA_UNET')))
